            ...
            <ul>
              ...
              <li class="mb-4" wire:key="{{ $category->id}} ">
                <label for="{{ $category-> slug}}" class="flex items-center dark:text-gray-400 ">
                  <input type="checkbox" wire:model.live="selected_categoies" id=" {{ $category->slug }} " value="{{ $category->id }}" class="w-4 h-4 mr-2">
             ...
             @foreach($products as $product)
               <div class="w-full px-3 mb-6 sm:w-1/2 md:w-1/3" wire:key="{{ $product->id }}">
                ...
                <div class="relative bg-gray-200">
                    ...
                    <img src="{{ url('storage/', $product->image_url) }}" alt="{{ $product->name }}" class="object-cover w-full h-56">
                ...
                <div class="p-4">
                    ...
                    <p class="text-xl font-semibold text-green-600 dark:text-green-500">
                ...
                <div class="flex justify-center p-4 border-t border-gray-300 dark:border-gray-700">
                    <a wire:click.prevent="addToCart({{ $product->id }})" href="#" class="text-gray-500 flex items-center space-x-2 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-300">
                        ...
                        <span wire:loading.remove wire:target='addToCart({{ $product->id }})'>
                          Add to Cart</span> <span wire:loading wire:target='addToCart({{ $product->id }})'>Adding...</span>
                    </a>
                </div>
            </div>
        </div>
